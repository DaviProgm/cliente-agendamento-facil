...
import { Clock, CalendarCheck, Ban, PlusCircle } from 'lucide-react';
    ...
    <div className="max-w-4xl mx-auto p-4 sm:p-6 bg-white rounded-2xl shadow-lg border border-gray-200">
      <div className="sticky top-0 z-20 bg-white/80 backdrop-blur-sm p-4 rounded-lg mb-6 flex items-center justify-between">
        ...
        <div className="text-center">
          <h2 className="text-xl md:text-2xl font-bold text-gray-800">{format(date, "EEEE, dd 'de' MMMM yyyy", { locale: ptBR })}</h2>
          <button
            ...
            return (
              <React.Fragment key={time}>
                ...
                <div
                  ...
                  className={`border-l-2 pl-4 py-3 relative ${event ? 'border-red-200' : isPast ? 'border-gray-200' : 'border-green-300 hover:border-green-500'}`}
                >
                  {event ? (
                    <div onClick={() => onEventClick?.(event)} className="bg-red-50 p-3 rounded-lg cursor-pointer hover:bg-red-100 transition-colors">
                      <p className="font-semibold text-red-800">{event.title}</p>
                      {event.description && <p className="text-sm text-red-700">{event.description}</p>}
